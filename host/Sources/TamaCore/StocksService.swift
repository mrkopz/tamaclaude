...
public final class StocksService {
    ...
    public func update(_ next: StockSettings) {
        ...
        unknown.removeAll()
        // ราคาชุดที่ถืออยู่เป็นของ watchlist ที่ไม่มีแล้ว จึงไม่ใช่ของที่ค้างได้ — ทิ้ง
        // เพื่อให้ `closed` มองว่ายังไม่เคยมีตัวเลข แล้วยิงหนึ่งรอบเดี๋ยวนี้
        //
        // ไม่งั้นการแก้ลิสต์ตอนตลาดปิดจะไม่มีผลจนกว่าจะถึงระฆังเปิด: จอจะโชว์หุ้นที่ผู้ใช้
        // เพิ่งลบไปทั้งคืน ทั้งที่ `/quote` คืนราคาปิดครั้งล่าสุดให้ได้อยู่แล้ว
        last.removeAll()
        toldClosed = false
        schedule.invalidate()
    }
