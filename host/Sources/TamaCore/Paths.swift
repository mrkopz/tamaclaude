...
public enum Paths {
    ...
    /// บัญชีที่งบจะนับ — ชื่อบัญชีบรรทัดละหนึ่ง (`claude`, `claude-work`, …)
    ///
    /// ไม่มีไฟล์ = นับทุกบัญชีที่ไม่ได้อยู่ในโควตา subscription ซึ่งเป็นสิ่งที่ถูก
    /// สำหรับคนที่มีบัญชีเดียว ส่วนคนที่แยกงานกับส่วนตัวเขียนชื่อบัญชีที่จ่ายเงินจริง
    /// ลงไปเพื่อให้แถบสะอาด — ป้ายบัญชีบอกได้แค่ว่า *ใคร* ไม่ได้บอกว่า *จ่ายไหม*
    /// คนที่รู้คำตอบนั้นคือเจ้าของเครื่อง
    public static var budgetAccounts: URL {
        stateDir.appendingPathComponent("budget-accounts")
    }
