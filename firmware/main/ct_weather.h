...
typedef struct {
    ...
    int age;
    // แถบพยากรณ์ห้าชั่วโมงข้างหน้า — `hour_n == 0` คือเฟรมนี้ไม่มีพยากรณ์มาด้วย
    // (host รุ่นก่อน หรือถูกบีบทิ้งเพราะชื่อเมืองยาว) ซึ่งไม่ใช่ความผิดพลาด
    //
    // `hour_start` เป็นชั่วโมงสัมบูรณ์ 0..23 ของช่องแรก ไม่ใช่ offset — บอร์ดไม่มี
    // timezone และเฟรมที่ค้างอยู่นับ offset ต่อเองไม่ได้ (ดู `WeatherFrame.hourStart`)
    int hour_start;
    int hour_n;
    int hour_temp[CT_WEATHER_FC_COLS];
    int hour_code[CT_WEATHER_FC_COLS];
} ct_weather_t;
