...
typedef struct {
    ...
    int age;
    // นาทีจนถึงนัดแรก ตอนที่ Mac สร้างเฟรม (`m`) — ลบได้ ศูนย์ได้ จึงต้องมีธงกำกับ
    // ว่า *มี* ค่าไหม แทนที่จะใช้ค่าพิเศษสักค่า (unknown != zero)
    //
    // บอร์ดนับถอยหลังต่อเองด้วย `age` ที่มีอยู่แล้ว ไม่ต้องมีเวลาสัมบูรณ์บนสายและไม่ต้อง
    // มีตัวจับเวลาใบใหม่ — เฟรมมาทุก 5 นาที ตัวเลขในเฟรมจึงเก่าเท่าอายุของมันเสมอ
    bool has_mins;
    int mins;
} ct_calendar_t;
