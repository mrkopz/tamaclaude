...
def build(raw: dict, size: int) -> tuple[list[Glyph], int, int]:
    ...
    return glyphs, *metrics(raw, size, glyphs)


def metrics(raw: dict, size: int, glyphs: list[Glyph]) -> tuple[int, int]:
    """(line_height, base_line) ของ *บรรทัดที่อาจมีภาษาไทย* ไม่ใช่ของ Sarabun ตัวเดียว

    วัดจาก glyph ที่ raster ออกมาแล้ว *หลัง* ใส่ระยะเลื่อนของ thai.toml — ไม่ใช่จาก
    `font.getmetrics()` ซึ่งไม่เคยเห็นระยะเลื่อนนั้นและเคยรายงานต่ำกว่าของจริง 8px
    ผลคือ `ู` ใต้ `ญ` ถูก LVGL หนีบทิ้งทั้งตัวบนบอร์ด (`กตัญญู` อ่านเป็น `กตัญญ`)
    ส่วน preview ซึ่งไม่หนีบ กลับวาดล้นออกนอกการ์ด — บั๊กเดียวให้ภาพคนละแบบสองฝั่ง

    รวมกับ metric ของฟอนต์หลักด้วย เพราะบรรทัดเดียวกันมีทั้งไทยและ ASCII
    ("ที่ปั๊มน้ำมัน 10:30") และ LVGL วางทั้งบรรทัดด้วยเส้นฐานเดียว
    """
    host = raw["font"]["host"][str(size)]
    ink = [g for g in glyphs if g.box_h]
    ascent = max([g.ofs_y + g.box_h for g in ink] + [host["line_height"] - host["base_line"]])
    descent = max([-g.ofs_y for g in ink] + [host["base_line"]])
...
def emit_json(size: int, glyphs: list[Glyph], line_h: int, base_l: int,
              host_ascent: int) -> str:
    return json.dumps(
        {
            ...
            "line_height": line_h,
            "base_line": base_l,
            # เส้นฐานของฟอนต์หลัก วัดจากขอบบนของกล่องบรรทัด — ที่ว่างเหนือมันคือที่ของ
            # วรรณยุกต์ไทย เลย์เอาต์วางป้ายด้วยขอบบนของตัวละติน ไม่ใช่ขอบบนของกล่อง
            "host_ascent": host_ascent,
            "bpp": BPP,
            "glyphs": {
                f"{g.cp:04X}": {
                    "adv": g.adv16 / 16,
...
def main() -> None:
    ...
    for size in raw["font"]["sizes"]:
        ...
        host = raw["font"]["host"][str(size)]
        host_ascent = host["line_height"] - host["base_line"]
        ...
        j_path.write_text(emit_json(size, glyphs, line_h, base_l, host_ascent),
                          encoding="utf-8")
