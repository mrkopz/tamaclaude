...
GLB_D = 6.5  # เส้นผ่านศูนย์กลาง — ใหญ่กว่าหัวครึ่งหนึ่ง จึงอ่านเป็นลูกโลกไม่ใช่ลูกปัด
GLB_CY = -2.2  # กึ่งกลางลูก: ขอบบน -5.45 (ไม่ล้น BOX_Y0) ขอบล่าง 1.05 (เหนือตาที่ 1.5)
GLB_PX = 0.25  # หนึ่งพิกเซลเป็นหน่วย unit ที่ unit_px = 4
GLB_BANDS = 13  # แถบแนวนอนที่ประกอบเป็นวงกลม — แถบละ 0.5 unit = 2 px
...
def _globe_bands() -> list[tuple[float, float, float]]:
    """แถบแนวนอนของลูกโลก (y0, y1, ครึ่งความกว้าง) — ทวีปใช้ค่าชุดเดียวกันในการตัดขอบ
    ...
    for i in range(GLB_BANDS):
        ...
        out.append((y0, y0 + bh, round(hw / GLB_PX) * GLB_PX))
    return out
