...
# แว่นตา — ท่านั่งพิมพ์ (writing) ไม่ใช่ prop ในช่อง PROPS เพราะติดหน้าไปกับตา
# ต้องยุบและเลื่อนไปพร้อมตาทุกเฟรม (mascot.py จึงวาดมันในชุดเดียวกับตา ไม่ใช่ชุด prop)
...
# บทเรียนเดียวกับเลนส์แว่นขยาย ถ้าค่าไหนตกครึ่งพิกเซล บันไดมุมจะเหลื่อมจนเห็นเป็นร่อง
GLS_S, GLS_T = 3.0, 0.5
# ขาแว่นยาวเท่าที่พอไปจบพอดีขอบลำตัว (2.0 และ 14.0) — เลยออกไปจะอ่านเป็นเส้นลอยข้างหัว
GLS_TEMPLE = 0.75


def glasses(eye_l: float, connected: bool = True) -> RectList:
    """แว่นตาคร่อมตาสองข้าง — วาดหลังตา (mascot.py เป็นคนเรียก) กรอบจึงทับขอบตา
    ...
    """
    col = _c(connected, PAL.steel)
    y = EYE_Y + EYE_S / 2.0 - GLS_S / 2.0
    xs = tuple(x + EYE_S / 2.0 - GLS_S / 2.0 for x in (eye_l, EYE_R))
    out: RectList = []
    for x in xs:
        out += _ring_round(x, y, GLS_S, GLS_T, col)
    ...
    by = y + GLS_S / 2.0 - GLS_T / 2.0
    out.append(Rect(xs[0] + GLS_S, by, xs[1] - xs[0] - GLS_S, GLS_T, col))
    out.append(Rect(xs[0] - GLS_TEMPLE, by, GLS_TEMPLE, GLS_T, col))
    out.append(Rect(xs[1] + GLS_S, by, GLS_TEMPLE, GLS_T, col))
    return out
